...
function Parser(filename, options) {
  ...
  if (self.offset > size) {
    throw new Error('File size is too small: requested offset ' + offset + ' but filesize is really: ' + size); 
  }
 ...
 * Unless `options.tail` is specified, this event will emit a 'nodata' event and the parser will terminate.
 *
 * @private
 */
Parser.prototype._wait_for_data = function() {
  ...
  self.emit('nodata', { position: self.last_read_position });
